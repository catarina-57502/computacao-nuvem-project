...
myClient = MongoClient('172.21.0.2', 27017 ,username='admin', password='admin' )
myDB = myClient["steam"]
myGames = myDB["Games"]
...
class SearchesService(searches_pb2_grpc.SearchesServicer):
    def SearchGames(self, request, context):
        ...
        docGame = myGames.find( {"$or": [{"name": request.name, "types": request.types, "release_date": request.release_date,
        "developer": request.developer, "publisher": request.publisher, "popular_tags": request.popular_tags,
        "languages": request.languages, "genre": request.genre, "original_price": request.original_price}]}).limit(3)
        games = []
        dict_game = {}
    ...
    def SearchGameById(self, request, context):
        ...
        return SearchReviewResponse(game=DocToGame(docGame))
    ...
    def SearchReviews(self, request, context):
        ...
        docReview = myReviews.find( {"$or": [{"app_name": request.app_name, "timestamp_created": request.timestamp_created,
        "recommended": request.recommended, "votes_helpful": request.votes_helpful, "votes_funny": request.votes_funny,
        "comment_count": request.comment_count}]}).limit(3)
